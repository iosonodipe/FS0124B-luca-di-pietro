...
let deleteOne = (stringa, bool) => {
  if (bool) return stringa.slice(1);
...
let onlyLetters = (stringa) => stringa.replace(/[0-9]/g, "");
...
let isThisAnEmail = (stringa) => {
  ...
  if (regex.test(stringa)) return true;
  else return false;
};
...
function howmManyDays(data) {
  let dataOdierna = new Date();
  let dataInserita = new Date(data);
  let diffData = dataOdierna.getTime() - dataInserita.getTime();
  regex = /\d{1,2}[\/\.\-]\d{1,2}[\/\.\-]\d{4}/g;

  if (regex.test(data)) {
    return Math.floor(diffData / 86400000);
  } else return "Inserisci un formato data corretto";
}
...
function isTodayMyBirthday() {
  let mioCompleanno = new Date("07/15/2024").toLocaleDateString();
  let dataOdierna = new Date().toLocaleDateString();
  if (mioCompleanno === dataOdierna) return true;
  else return false;
}
...
function searchByTitle(stringa) {
  let filmMatch = movies.filter((film) =>
    film.Title.toLowerCase().includes(stringa)
  );
  return filmMatch;
}
...
let selezionaContainer = () => document.querySelector("#container");
